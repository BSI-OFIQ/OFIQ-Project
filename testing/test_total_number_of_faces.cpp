...
TEST(TotalNumberOfFaces, SSD)
{
  ...
  std::string inputImageFile = jsonValue["Test.TotalFacesPresent"]["input_image"].get_string();
  auto rawScore = jsonValue["Test.TotalFacesPresent"]["raw"].get_double();
  auto scalarScore = jsonValue["Test.TotalFacesPresent"]["scalar"].get_double();
  ...
  {
      if (qaResult.first == FACEQA::QualityMeasure::TotalFacesPresent )
      {
