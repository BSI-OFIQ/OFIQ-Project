...
TEST(MouthOpen, WD5)
{
  ...
  std::string inputImageFile = jsonValue["Test.MouthOpen"]["input_image"].get_string();
  auto rawScore = jsonValue["Test.MouthOpen"]["raw"].get_double();
  auto scalarScore = jsonValue["Test.MouthOpen"]["scalar"].get_double();
  ...
  {
      if (qaResult.first == FACEQA::QualityMeasure::MouthOpen )
      {
